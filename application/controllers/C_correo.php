...
class C_correo extends CI_Controller {
    ...
    // Enviar código de autenticación para 2FA
    public function enviar_codigo_autenticacion($usuario) {
        $mensaje = "
            <!DOCTYPE html>
            <html lang='es'>
            <head>
                    ...
                    <tr>
                        <td align='center' style='padding: 40px 0;'>
                            <table width='600' cellpadding='0' cellspacing='0' style='background-color: #ffffff; border-radius: 8px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);'>
                                <tr>
                                    <td style='background-color: #007bff; padding: 30px; text-align: center; border-radius: 8px 8px 0 0;'>
                                        <h1 style='color: #ffffff; margin: 0; font-size: 24px;'>Código de Autenticación</h1>
                                    </td>
                                </tr>
                                <tr>
                                    <td style='padding: 40px 30px;'>
                                        <p style='color: #333333; font-size: 16px; margin-bottom: 20px;'>Hola <strong>".$usuario->name."</strong>,</p>
                                        <p style='color: #555555; font-size: 14px; line-height: 1.6; margin-bottom: 15px;'>
                                            Has solicitado iniciar sesión. Tu código de autenticación es:
                                        </p>
                                        <div style='background-color: #f8f9fa; border: 1px solid #dee2e6; border-radius: 6px; padding: 20px; margin: 25px 0; text-align:center;'>
                                            <span style='font-size: 32px; color: #007bff; letter-spacing: 4px; font-weight: bold;'>".$usuario->auth_code."</span>
                                        </div>
                                        <p style='color: #555555; font-size: 14px; line-height: 1.6; margin-bottom: 25px;'>
                                            Este código es válido por 2 horas. Si no fuiste tú quien solicitó este acceso, ignora este correo.
                                        </p>
                                    </td>
                                </tr>
                                <tr>
        ...
        $this->email->subject('Código de autenticación para inicio de sesión');
        $this->email->message($mensaje);
        $this->email->send();
    }
}